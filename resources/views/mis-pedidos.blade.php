...
<div class="container-custom">
    ...
    <div class="header-compact">
        ...
        <div class="header-stats">
            <span class="stat-badge"><i class="fas fa-shopping-bag"></i> {{ $pedidos->count() }} pedidos</span>
            <a href="{{ route('landing') }}" class="stat-badge" style="text-decoration: none;">
    ...
    @if($pedidos->isEmpty())
        <div class="alert-empty">
            ...
            <a href="{{ route('landing') }}" class="btn-tienda">
        ...
        @foreach($pedidos as $pedido)
            ...
            <div class="pedido-card">
                <div class="pedido-header">
                    <div>
                        <span class="pedido-id">#{{ $pedido->id_nota_venta }}</span>
                        <span class="pedido-fecha ms-2">
                            <i class="far fa-calendar-alt"></i> {{ \Carbon\Carbon::parse($pedido->fecha_venta)->format('d/m/Y H:i') }}
                        </span>
                    </div>
                    @if($pagado)
                ...
                <div class="pedido-body">
                    <div class="productos-compact">
                        @foreach($pedido->detalles as $detalle)
                            <span class="producto-tag">{{ $detalle->cantidad }}x {{ $detalle->item->nombre ?? 'Producto' }}</span>
                        @endforeach
                    </div>
                    <div class="total-row">
                        <span class="total-label">Total</span>
                        <span class="total-amount">Bs. {{ number_format($pedido->monto_total, 2) }}</span>
                    </div>

                    @if(!$pagado && !$cancelado)
                        <div class="acciones">
                            <a href="{{ route('pago.reintentar', $pedido->id_nota_venta) }}" class="btn-sm-custom btn-pagar-sm">
                                <i class="fas fa-credit-card"></i> Pagar
                            </a>
                            <form action="{{ route('mis-pedidos.cancelar', $pedido->id_nota_venta) }}" 
                                  method="POST" 
