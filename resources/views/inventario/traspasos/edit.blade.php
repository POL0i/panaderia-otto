...
<div class="container-fluid">
    <div class="row mb-3 animate-fade-in-up">
        <div class="col-md-8">
            <h1 class="h3 mb-0"><i class="fas fa-edit icon-panaderia"></i> Editar Traspaso #{{ $traspaso->id_traspaso }}</h1>
        </div>
    ...
    <div class="card shadow-sm animate-fade-in-up">
        ...
        <form action="{{ route('traspasos.update', $traspaso) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-md-6">
                        ...
                        <select name="id_almacen_origen" id="id_almacen_origen" class="form-control @error('id_almacen_origen') is-invalid @enderror">
                            ...
                            @foreach($almacenes as $almacen)
                                <option value="{{ $almacen->id_almacen }}" {{ old('id_almacen_origen', $traspaso->id_almacen_origen) == $almacen->id_almacen ? 'selected' : '' }}>
                                    {{ $almacen->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="id_almacen_destino" id="id_almacen_destino" class="form-control @error('id_almacen_destino') is-invalid @enderror">
                            <option value="">Seleccione un almacén</option>
                            @foreach($almacenes as $almacen)
                                <option value="{{ $almacen->id_almacen }}" {{ old('id_almacen_destino', $traspaso->id_almacen_destino) == $almacen->id_almacen ? 'selected' : '' }}>
                                    {{ $almacen->nombre }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <select name="id_item" id="id_item" class="form-control @error('id_item') is-invalid @enderror">
                            ...
                            @foreach($items as $item)
                                <option value="{{ $item->id_item }}" {{ old('id_item', $traspaso->id_item) == $item->id_item ? 'selected' : '' }}>
                                    {{ $item->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="number" name="cantidad" id="cantidad" step="0.01" class="form-control @error('cantidad') is-invalid @enderror" value="{{ old('cantidad', $traspaso->cantidad) }}">
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="number" name="precio_unitario" id="precio_unitario" step="0.01" class="form-control @error('precio_unitario') is-invalid @enderror" value="{{ old('precio_unitario', $traspaso->precio_unitario) }}">
                ...
                <div class="form-group">
                    ...
                    <textarea name="observaciones" id="observaciones" class="form-control @error('observaciones') is-invalid @enderror" rows="3">{{ old('observaciones', $traspaso->observaciones) }}</textarea>
