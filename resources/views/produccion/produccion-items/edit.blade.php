...
@section('title', 'Editar Producción Item Almacén')

@section('content')
<div class="container-fluid">
    <div class="row mb-3 animate-fade-in-up">
        <div class="col-md-8">
            <h1 class="h3 mb-0"><i class="fas fa-edit icon-panaderia"></i> Editar Asignación</h1>
        </div>
    ...
    <div class="card shadow-sm animate-fade-in-up">
        ...
        <form action="{{ route('produccion-items.update', $produccionItem) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <select name="id_produccion" id="id_produccion" class="form-control @error('id_produccion') is-invalid @enderror">
                            ...
                            @foreach($producciones as $produccion)
                                <option value="{{ $produccion->id_produccion }}" {{ old('id_produccion', $produccionItem->id_produccion) == $produccion->id_produccion ? 'selected' : '' }}>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select name="id_almacen" id="id_almacen" class="form-control @error('id_almacen') is-invalid @enderror">
                            ...
                            @foreach($almacenes as $almacen)
                                <option value="{{ $almacen->id_almacen }}" {{ old('id_almacen', $produccionItem->id_almacen) == $almacen->id_almacen ? 'selected' : '' }}>
                                    {{ $almacen->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select name="id_item" id="id_item" class="form-control @error('id_item') is-invalid @enderror">
                            ...
                            @foreach($items as $item)
                                <option value="{{ $item->id_item }}" {{ old('id_item', $produccionItem->id_item) == $item->id_item ? 'selected' : '' }}>
                                    {{ $item->nombre }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="cantidad_producida" id="cantidad_producida" step="0.01" class="form-control @error('cantidad_producida') is-invalid @enderror" value="{{ old('cantidad_producida', $produccionItem->cantidad_producida) }}" required>
