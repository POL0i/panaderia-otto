...
@section('title', 'Editar Detalle de Receta')

@section('content')
<div class="container-fluid">
    <div class="row mb-3 animate-fade-in-up">
        <div class="col-md-8">
            <h1 class="h3 mb-0"><i class="fas fa-edit icon-panaderia"></i> Editar Detalle de Receta</h1>
        </div>
    ...
    <div class="card shadow-sm animate-fade-in-up">
        ...
        <form action="{{ route('detalles-receta.update', $detalleReceta) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-md-6">
                        ...
                        <select name="id_receta" id="id_receta" class="form-control @error('id_receta') is-invalid @enderror">
                            ...
                            @foreach($recetas as $receta)
                                <option value="{{ $receta->id_receta }}" {{ old('id_receta', $detalleReceta->id_receta) == $receta->id_receta ? 'selected' : '' }}>
                                    {{ $receta->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="id_insumo" id="id_insumo" class="form-control @error('id_insumo') is-invalid @enderror">
                            ...
                            @foreach($insumos as $insumo)
                                <option value="{{ $insumo->id_insumo }}" {{ old('id_insumo', $detalleReceta->id_insumo) == $insumo->id_insumo ? 'selected' : '' }}>
                                    {{ $insumo->nombre }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <input type="number" name="cantidad_requerida" id="cantidad_requerida" step="0.01" class="form-control @error('cantidad_requerida') is-invalid @enderror" value="{{ old('cantidad_requerida', $detalleReceta->cantidad_requerida) }}" required>
