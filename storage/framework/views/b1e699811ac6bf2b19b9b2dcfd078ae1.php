    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-warning">
                <h5 class="modal-title">Editar Cantidad y Unidad</h5>
...
<div class="modal fade" id="confirmDeleteInsumoModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>¿Está seguro de que desea eliminar este insumo de la receta?</p>
                <p class="text-muted mb-0">
                    <i class="fas fa-info-circle"></i> 
                    Si el insumo ya fue usado en producciones, no podrá ser eliminado.
                </p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
                <button type="button" class="btn btn-danger" id="btnConfirmDeleteInsumo">
...
$(document).ready(function() {
    ...
    function updateSelectedCount() {
        ...
        $('#selectedCount').text(count + ' insumo(s) seleccionado(s)');
    ...
    $('#formAddInsumos').on('submit', function(e) {
        ...
        $('.insumo-checkbox:checked').each(function() {
            ...
            if (!cantidad || parseFloat(cantidad) <= 0) {
                toastr.error('Todos los insumos seleccionados deben tener una cantidad válida');
                return false;
            }
        ...
        if (insumos.length === 0) {
            toastr.error('Debe seleccionar al menos un insumo con cantidad');
            return;
        }
        ...
        $.ajax({
            ...
            error: function(xhr) {
                var msg = 'Error al agregar insumos';
    ...
    let detalleToDelete = null;

    // Abrir modal de confirmación
    $(document).on('click', '.btn-delete-detalle', function() {
        detalleToDelete = {
            id: $(this).data('id'),
            card: $('#detalle-' + $(this).data('id'))
        };
        $('#confirmDeleteInsumoModal').modal('show');
    });

    // Confirmar eliminación
    $('#btnConfirmDeleteInsumo').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                toastr.success(response.message);
                detalleToDelete.card.fadeOut(300, function() {
                    $(this).remove();
            ...
            error: function(xhr) {
                var msg = 'Error al eliminar el insumo';
                
                if (xhr.status === 422 && xhr.responseJSON?.message) {
                    msg = xhr.responseJSON.message;
                    toastr.warning(msg, 'No se puede eliminar', {
                        timeOut: 10000,
